...
class BaseICAPRequestHandler(StreamRequestHandler):
    ...
    def cont(self):
        ...
        if self.cont_send:
            raise ICAPError(500, 'Tried to continue a second time!')
    ...
    def send_150_scanning_inprogress(self):
        '''
        ICAP/1.0 150 Scan In Progress
        Server: DLP appliance
        Date: Tue, 22 Jun 2021 14:41:23 GMT
        ISTag: "04ff6bb1-4ebc-457a-8a30-14fd6434b840-1387"
        ...
        self.wfile.write(b'ICAP/1.0 150 Scanning In Progress\r\n')
        header_list = [b'ISTag', b'Date', b'Server']        
        for header in header_list:
            logging.info("Write: %s : %s", header, self.icap_headers[header][0])
            self.wfile.write(header + b': ' + self.icap_headers[header][0] + b'\r\n')
        self.wfile.write(b'Encapsulated: null-body=0\r\n')
        self.wfile.write(b'\r\n')

    def send_headers(self, has_body=False, no_resphdr=False, skip_headers=""):
        ...
        if no_resphdr:
            body_offset = 0
            enc = enc_body + str(body_offset).encode('utf-8')
        ...
        icap_header_str = b''
        icap_header_skip_options = {
            "noencap" : b'Encapsulated',
            "noistag": b'ISTag',
            "noserver": b'Server',
            "nodate" : b'Date',
        }
        icap_header_skip_list = []
        skip_list = []
        if skip_headers is not None:
            skip_list = skip_headers.split(';')
        for s in skip_list:
            if s in icap_header_skip_options:
                icap_header_skip_list.append(icap_header_skip_options[s])
                logging.debug("Add to skip header list: %s", icap_header_skip_options[s])

        # just randomize the icap_headers
        icap_headers_keys = list(self.icap_headers.keys())
        random.shuffle(icap_headers_keys)
        for k in icap_headers_keys:
            if k in icap_header_skip_list:
                logging.debug("Skip this header: %s", k)
                continue
            ...
            for v in self.icap_headers[k]:
                ...
                logging.debug("ICAP Header: '%s:%s'", k, v)

        icap_header_str += b'\r\n'
        response_line = self.icap_response
        if "nostatus" in skip_list:
            logging.debug("Skip status line: '%s'", response_line)
            response_line = b''
    ...
    def parse_request(self):
        ...
        self.scan_intv = self.headers.get(b'x-scan-progress-interval', [None])[0]
        self.allow = [
